 ...
 */
class Freephabet extends FlxSpriteGroup
{
	...
	var lastSprite:FreeCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public var isBold:Bool = false;
	public var lettersArray:Array<FreeCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = FreeCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = FreeCharacter.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = FreeCharacter.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:FreeCharacter = new FreeCharacter(30 * loopNum, 0, textSize);
				var letter:FreeCharacter = new FreeCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = FreeCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = FreeCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = FreeCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:FreeCharacter = new FreeCharacter(30 * loopNum, 0, textSize);
				var letter:FreeCharacter = new FreeCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			var scaledY = FlxMath.remapToRange(targetY, 0, 0.67, 0, 2); //(targetY, 0, THIS ONE CHANES THE SPACING|ADD MORE TO LOWER SPACING, 0, ALSO SOME SPACING|ADD MORE FOR HIGHER SPACING)
			...
			if(forceX != Math.NEGATIVE_INFINITY) {
				...
				x = FlxMath.lerp(x, (targetY * 1) + 90 + xAdd, lerpVal);
			}
		}
...
class FreeCharacter extends FlxSprite
{
