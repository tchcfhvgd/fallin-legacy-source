...
class PlayState extends MusicBeatState
{
	...
	public static var isFallStage:Bool = false;
	...
	override public function create()
	{
		...
		isFallStage = stageData.isFallStage;
		...
		{
			case 'theWhirlygig':
				var whirlySky:BGSprite = new BGSprite('fallmen/Stages/sky', -1000, -500, 0, 0);
				whirlySky.setGraphicSize(Std.int(whirlySky.width * 0.8));
				whirlySky.updateHitbox();
				add(whirlySky);

				var whirlyBackdrop:BGSprite = new BGSprite('fallmen/Stages/Whirlygig/wback', -1100, -600, 0.3, 0.3);
				whirlyBackdrop.setGraphicSize(Std.int(whirlyBackdrop.width * 0.9));
				whirlyBackdrop.updateHitbox();
				add(whirlyBackdrop);

				var whirlyGround:BGSprite = new BGSprite('fallmen/Stages/Whirlygig/wground', -800, 620);
				add(whirlyGround);
				
				whirlyCrowd = new BGSprite('fallmen/Stages/Whirlygig/wbop', -300, 140, 1, 1, ['fallbopw']);
				whirlyCrowd.setGraphicSize(Std.int(whirlyCrowd.width * 1));
				whirlyCrowd.updateHitbox();
				add(whirlyCrowd);

			case 'seeSaw':
				var seeSky:BGSprite = new BGSprite('fallmen/Stages/sky', -1000, -500, 0, 0);
				seeSky.setGraphicSize(Std.int(seeSky.width * 0.8));
				seeSky.updateHitbox();
				add(seeSky);

				var seeBackdrop:BGSprite = new BGSprite('fallmen/Stages/SeeSaw/ssback', -1100, -600, 0.8, 0.8);
				seeBackdrop.setGraphicSize(Std.int(seeBackdrop.width * 0.9));
				seeBackdrop.updateHitbox();
				add(seeBackdrop);

				var seeGround:BGSprite = new BGSprite('fallmen/Stages/SeeSaw/ssground', -800, 292);
				add(seeGround);
				
				seeCrowd = new BGSprite('fallmen/Stages/SeeSaw/ssbop', -300, 140, 1, 1, ['fallbopss']);
				seeCrowd.setGraphicSize(Std.int(seeCrowd.width * 1));
				seeCrowd.updateHitbox();
				add(seeCrowd);
				
			case 'tipToe':
				var tipSky:BGSprite = new BGSprite('fallmen/Stages/sky', -1000, -500, 0, 0);
				tipSky.setGraphicSize(Std.int(tipSky.width * 0.8));
				tipSky.updateHitbox();
				add(tipSky);

				var tipBackdrop:BGSprite = new BGSprite('fallmen/Stages/TipToe/ttback', -1100, -600, 0.8, 0.8);
				tipBackdrop.setGraphicSize(Std.int(tipBackdrop.width * 0.9));
				tipBackdrop.updateHitbox();
				add(tipBackdrop);

				var tipGround:BGSprite = new BGSprite('fallmen/Stages/TipToe/ttground', -800, 292);
				add(tipGround);
				
				tipCrowd = new BGSprite('fallmen/Stages/TipToe/ttbop', -300, 140, 1, 1, ['fallboptt']);
				tipCrowd.setGraphicSize(Std.int(tipCrowd.width * 1));
				tipCrowd.updateHitbox();
				add(tipCrowd);
				
			case 'theRing':
				var ringBackground:BGSprite = new BGSprite('fallmen/Stages/RapBattle/ring', -435, -50, 1, 1);
				ringBackground.setGraphicSize(Std.int(ringBackground.width * 1));
				ringBackground.updateHitbox();
				add(ringBackground);

				calobiWatermark = new BGSprite('fallmen/Stages/RapBattle/Calobi', 1110, 0, 0, 0);
		...
		if (curSong == 'Rap Battle' && !FlxG.save.data.DemoFreeplaySong4)
		{
			Sys.exit(0);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'bombshell':
					FlxG.save.data.DemoFreeplaySong2 = true;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (PlayState.isFallStage)
		{
			pixelShitPart1 = 'fallmen/';
			pixelShitPart2 = '';
		}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				introAssets.set('fall', ['fallmen/starting', 'fallmen/ready', 'fallmen/set', 'fallmen/go']);
				...
				switch (swagCounter)
				{
					case 0:
						...
						FlxTween.tween(countdownStarting, {/*y: countdownStarting.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						FlxG.save.data.DemoFreeplaySong3 = true;
						...
						if (!FlxG.save.data.DemoScreenSeen)
						{
			...
			else
			{
				...
				{
					if (!FlxG.save.data.DemoBeatRapBattle)
					{
						MusicBeatState.switchState(new CreditsState());
						CoolUtil.browserLoad("https://www.youtube.com/watch?v=orjj9aeF3sY");
						trace("YOU BEAT MONGUS");
						FlxG.save.data.DemoBeatRapBattle = true;
					}
					else
					{
